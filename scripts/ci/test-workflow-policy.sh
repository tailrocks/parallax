...
rg '^    needs: \[changes, actionlint, source-hygiene, security-hygiene, policy, fmt, docs-links, audit, check, clippy, test, ui, ui-formatter-platform, embed, native-macos, closure-final\]$' "$workflows/ci.yml" >/dev/null || {
...
rg -U '^  closure-final:\n(?:.*\n)*?    permissions:\n      contents: read\n(?:.*\n)*?          install_args: "rust bun actionlint cargo:cargo-nextest cargo:cargo-audit cargo:cargo-deny cargo:cargo-hack cargo:cargo-shear"\n(?:.*\n)*?            scripts/ci/closure-baseline.sh\n            cargo xtask closure-final$' "$workflows/ci.yml" >/dev/null || {
