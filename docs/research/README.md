...
- [v1-scope.md](architecture/v1-scope.md) — the V1 definition (operator statement #6): the self-sufficient local machine, exhaustively — install/engine auto-download, ingest, processing, the `parallax run start -- <cmd>` wrapper centerpiece, the complete CLI list, retention defaults, docs, the out-of-scope table, the build checklist, and V1 risks. V1 done = the operator's daily development runs through it.
