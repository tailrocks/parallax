...
- [v1-build-plan.md](architecture/v1-build-plan.md) — historical V1 sequencing record (operator statement #5): goals 1+2, workspace/crate layout, milestones M0–M6, and dogfood exit criteria. It is not an active implementation plan; unfinished residuals live in research decisions.
...
- [v1-scope.md](architecture/v1-scope.md) — the V1 definition (operator statement #6, revised for #7): the self-sufficient local machine, shipped delivery inventory, acceptance scenarios, exclusions, and risks. Residual implementation ownership lives in research decisions.
- [v1-implementation-spec.md](architecture/v1-implementation-spec.md) — the concrete V1 contracts: workspace conventions, dependency set, ports, `config.toml`, GreptimeDB + Turso DDL, OTLP mapping, GraphQL SDL, UI page/query map, CLI output, and engine supervision. Contract changes land here first, then in code; unfinished residuals live in research decisions.
...
- [build-roadmap.md](architecture/build-roadmap.md) — historical de-risking sequence and research-gate rationale; active implementation ownership is only in shipped `crates/` / `ui/` plus research decisions.
...
- [streaming/](storage/streaming/) — historical broker candidate evaluation and a dormant replay/backpressure experiment protocol; active work is owned in shipped source plus research decisions.
...
- [competitor-pain-points.md](market/competitor-pain-points.md) — **user-voice pain-point corpus** (2026-08-14): what real users complain about per cohort (Sentry+lightweights, incumbents, OSS challengers, practitioner surveys), the two structural roots, and the pain → Parallax state → action mapping that seeded `plans/173`–`176`. First user-sourced evidence in the market folder (everything prior was analyst-derived).
