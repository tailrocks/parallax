...
Pins: GreptimeDB **`v1.1.3`** (`63ef18a7…`), ClickHouse **`v26.6.1.1193-stable`**
(`840482cd…`) — Run 205 pin refresh (2026-07-17). Historical smoke rows below retain
their original measurement pins unless a later Run re-verified.
...
| **Logs** · full-text / substring search (**selective**) | **~2× ClickHouse (corrected — was reported ~18×)** | **Runs 48–49 corrected it**: the ~18× (Run 12) was a **backend/function mismatch** — `matches()` (tantivy query-syntax fn) on a `backend='bloom'` index doesn't push to the index → **full-scans 5M** (~150 ms, EXPLAIN `output_rows: 5000000`). With the **correct pairing** GreptimeDB prunes (`output_rows: 1`): tantivy+`matches()` **~6 ms** or bloom+`matches_term()` **~8 ms**, vs CH `hasToken`/`text` **~3 ms** = **~2× warm, both sub-perceptible**. Not an index-maturity gap. **Re-verified Run 78 (no drift): CH ~3 ms vs GT bloom+`matches_term` ~8 ms (~2.7×); the wrong pairing `matches()`-on-bloom still full-scans ~157 ms — the 18× artifact reproduces.** **Re-verified Run 157 (`EXPLAIN`, 5M `logs_b1`, exec — plan-level, no drift):** a maximally-selective UUID-fragment (1 match in 5M) prunes on **both** — GT bloom FULLTEXT 5M→**~51,200 rows read** (bloom granularity 10240 ≈ 5 blocks) vs CH `text` index 5M→**`Granules 1/611`** (~8,192 rows); broad terms (`timeout` 699k, `503` 232k) prune on **neither** (every 8192-row granule contains matches → `Granules 611/611`, GT scans 5M) → scan-bound where CH's vectorized engine wins. CH's finer granule (8,192 vs GT's ~51,200) is the slight selective edge — both tiny reads. Mechanism holds. | selective token search | **measured warm (Runs 48–49, 78); plan re-verified (Run 157)** |
...
| **Traces** · `trace_id` point lookup | schema-decided; gap small in absolute ms | Sort-key prefix locality: ClickHouse `ORDER BY (trace_id, ts)` → sparse index seeks **Granules: 1** (Run 2), **2 ms**. GreptimeDB seed PK `(service,name)` leaves `trace_id` un-keyed → scan; **fair server-time = 14 ms** (Run 40 — *not* the 54 ms HTTP-wall; the ~40 ms was HTTP floor), **~8 ms with the `trace_id INVERTED INDEX` Parallax's design adds** (Run 6). So CH ~4–7× by locality, but **both ≪ 300 ms gate** — not latency-bound. **Flips toward ~tie when GreptimeDB keys/indexes `trace_id`** (its design does). **Re-verified Run 158 (`EXPLAIN`, exec — the dominant-query pillar, both engines): anchored `trace_id` fetch prunes IFF `trace_id` is keyed/indexed on that signal.** Keyed: GreptimeDB `spans_idx` (`trace_id` INVERTED) reads **14 rows**; ClickHouse `spans` (`ORDER BY (trace_id,ts)`) **`Granules 1/123`**. Un-keyed: on `logs_b1` (both keyed `service`, not `trace_id`) **both full-scan** — ClickHouse `Granules 611/611`; GreptimeDB `UnorderedScan` `scan_cost 429ms` over 49 file_ranges (emits 1, reads 5M). → **Schema blueprint (engine-agnostic): key/index `trace_id` (or `fingerprint`) on EVERY signal table** or the anchored bundle scans. *Methodological note:* GreptimeDB scan `output_rows` is **post-filter emission, not rows-read** — gauge scan work by `scan_cost`/`file_ranges`, not `output_rows` (it can mask a full scan). | Whoever keys `trace_id` wins; absolute ms tiny | plan+smoke (Runs 2/6/40, **158**) |
| **Traces** · status/duration filter, span tree over window | ClickHouse (slight) on scan; **tree build capability ~parity** | Vectorized columnar scan + granule skip; GreptimeDB competitive via DataFusion. **Run 195:** recursive CTE **JOIN form works on GT v1.1.3** (comma self-join still fails). App-side tree optional. | Analytical scan / tree | smoke (Run 195) |
