...
| [Sentry MCP repository](https://github.com/getsentry/sentry-mcp) and [Sentry MCP releases](https://github.com/getsentry/sentry-mcp/releases) | Sentry says its MCP service targets human-in-the-loop coding agents rather than every Sentry API. The checked release page shows `0.35.0` on 2026-05-21. The README describes stdio for self-hosted Sentry as work in progress and lists token scopes including `project:write`, `team:write`, and `event:write`; AI-powered search needs OpenAI or Anthropic provider configuration. | Sentry is not simply a broad admin MCP, but the self-hosted stdio path is not read-only by documented scope. Parallax should not claim MCP parity until it proves narrower scopes and bundle projection. |
