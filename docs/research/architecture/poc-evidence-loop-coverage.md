...
| 13 | Draft JSON Schemas + emitted-artifact validation (`schema/`) | [integration-contract.md](integration-contract.md) | **A3 schema gate** — PoC schemas under `poc/…` remain drafts; **product** schemas ship at repo-root `schema/evidence-bundle.v{1,2}.schema.json` with tests (pass 60: `schema_artifacts_shipped`). External adoption still open | schema_artifacts_shipped / adoption open |
