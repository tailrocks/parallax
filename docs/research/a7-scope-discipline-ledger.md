...
| [Traceway embedded mode](https://docs.tracewayapp.com/learn/embedded-mode) | Traceway can run an embedded development server inside a Go process with SQLite for local observability. This pressures Parallax's local developer experience. |
| [Traceway all-in-one container](https://docs.tracewayapp.com/server/all-in-one) | Traceway offers one container bundling backend, ClickHouse, and Postgres. Even when internals are heavier, competitors can hide setup complexity behind one artifact. Parallax should count both services and hidden bundled subsystems. |
