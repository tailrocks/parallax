...
| OpenObserve | AGPL + Self-Hosted Enterprise | **Pass 94 primary [pricing FAQ](https://openobserve.ai/pricing/):** Self-Hosted Enterprise free ≤ **`50 GB/day`** (two FAQ answers; prior `200 GB/day` conflict treated **stale/resolved**). Enterprise includes SSO/RBAC/audit/**SDR**, AI SRE Agent, AI Assistant, AI-powered observability. Cloud Professional **$0.50/GB ingest**, **$0.01/GB query**. | Copy the ops gating; **do NOT copy the agent gating** — that is the weakness Parallax beats. Free allowance pin = **50 GB/day**. |
