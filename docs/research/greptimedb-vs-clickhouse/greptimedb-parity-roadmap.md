...
| 3 | PREWHERE late materialization | **Engineering — PARTIALLY SHIPPED (Run 121)** | GreptimeDB **v1.0.2 added its own `prefilter.rs` framework** (read filter columns first → refined row selection → read the rest), wired into the Flat read path (`file_range.rs: has_flat_primary_key_prefilter`); currently PK/partition-column-scoped. General wide-column PREWHERE is the remaining piece (Tier B). | No |
