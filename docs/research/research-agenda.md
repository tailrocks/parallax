...
Last updated 2026-07-17 after pass 86 (Apache Iggy still **`server-0.8.0`**;
clustering issue #2562 closed **for inactivity**, not HA GA; multi-shard docs =
single-node; product Iggy still deferred). Pass 85 = commoditization. A1/A2/A4/A6 open.
...
| **2** | **Is there a sustainable *paying* segment, and what is the product that captures it?** (A2 + business model) | **#1 business risk.** Open self-hosted is structurally non-paying; survivors monetized via managed cloud / enterprise-gating. | Desk + interviews. | **Segment sized + monetization shape designed (2026-05-29)**; **desk recheck 2026-07-17 pass 54:** hard-boundary thesis + Seer-not-self-hosted + OSS survivor cloud/EE pattern still hold; **A2 interviews still open** (desk cannot size). | [validation/monetization-and-paying-segment.md](validation/monetization-and-paying-segment.md), [validation/a2-user-demand.md](validation/a2-user-demand.md), [validation/business-model.md](validation/business-model.md) |
...
| **5** | **Sized storage cost + cold-read latency + self-host-vs-cloud + current stable re-test** | Characterizes the mandatory GreptimeDB engine. **Lower priority** — storage was never the existential risk. | Server-tier benchmark (cannot run in the dev capsule). | **Blocked/deferred for measurement.** Pins **`v1.1.3`** / nightly **`v1.2.0-nightly-20260706`**. **Pass 82:** TTL *mechanism* (whole-SST drop, compaction-gated physical reclaim; CH default row-level) re-confirmed on current source/docs — does **not** close sized $/GB or cold-read gates. Performance/cost numbers still **unproven** until benchmark agent re-runs against v1.1.3. Mechanism note: [storage/greptimedb-vs-clickhouse/retention-and-ttl.md](storage/greptimedb-vs-clickhouse/retention-and-ttl.md). | [decisions/storage-engine.md](decisions/storage-engine.md), [storage/size-and-object-cost.md](storage/size-and-object-cost.md), [retention-and-ttl.md](storage/greptimedb-vs-clickhouse/retention-and-ttl.md) |
