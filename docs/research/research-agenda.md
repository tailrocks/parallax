...
Last updated 2026-07-18 after pass 242 (monetization triangle **holds**; Grafana
Assistant **$20/AI user + token meters** pinned; SigNoz Noz still **Cloud**; O2
EE AI/SDR + **≤50 GB/day** free self-host EE; A2 interviews still **zero rows**).
Pass 241 = wedge cohort. A1/A2/A4 open; A6 open at mixed gate only.
...
- **Engine releases** — re-pin + re-verify load-bearing claims on each new stable (GreptimeDB; ClickHouse feature line). Last: **2026-07-18 pass 238 (API only, no bench)** — GreptimeDB stable **`v1.1.3`** (GitHub **Latest**); latest *named* nightly **release** tag still **`v1.2.0-nightly-20260706`**. ClickHouse **feature** line still **`v26.6.1.1193-stable`**. Server-tier size/cost measurement still owed (agenda #5). **Traces docs GA:**
