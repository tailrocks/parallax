...
| Prompt injection via attacker-controlled telemetry (inject-*in*) | Safety | H | M | Injection canary in a log/exception/attribute field steers the agent or exfiltrates in red-team | Trust boundary + provenance tiers; structured-not-freetext; read-only context with no exfil vector; never-execute-from-data; separate sandboxed human-gated fixer — see [agent trust boundary & prompt injection](../architecture/agent-trust-boundary-and-prompt-injection.md) |
