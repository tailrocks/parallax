...
| Claim | Status | In-repo pointer | Notes |
| --- | --- | --- | --- |
| GraphQL query API | **shipped** | `crates/parallax-api/`; exported schema `ui/graphql/schema.graphql` | **81** Query fields, **14** Mutation fields (R3+R5 restamp: `rumSessions` + `rumSession` + `releaseHealth` new, 78→81; audit-M1 restamp: `chartAnnotations` is new in PR71 `ec0eea1c`, 77→78; `service` is now optional). **Count method (SoT):** walk `type Query` / `type Mutation` in the generated SDL; **skip `"""…"""` description blocks** (and single-line `"` descriptions); then match `fieldName(` or `fieldName:`. A naive line regex that does **not** skip descriptions falsely reports extra fields. Re-count before changing this row. `chartAnnotations(fromNanos, toNanos, service: Option)` — omit service collects ≤32; kind `release`, title=version. Trace field `dominantDbQueries`. `releaseHealth(service, fromNanos, toNanos)` — per-release crash-free session/user rates + `suspectRelease` flag. |
...
| Browser RUM surface | **partial** | `ui/src/features/rum/`, route `/rum/` | Session inbox + timeline over `rumSessions` / `rumSession` (R3) alongside the vitals/errors/journeys projection over `tracesPage` / `histogramQuantile` / `issues`. No source maps. |
...
| Sampling policy / source maps | **absent** | — | Freeze P0 remaining-gaps. Do not claim shipped. |
...
| 2026-09-14 | R5 release health: GraphQL row **78→81** (with R3) — `releaseHealth` is new (per-service+version crash-free session/user rates + `suspectRelease` on crash-rate regression vs previous release). Mutation 14 holds. Rollup reuses `serviceVersion`/error-event plumbing (`ReleaseHealth::from_counts`, `flag_suspect_releases` shared by memory + GreptimeDB); strip shows crash-free % + suspect badge. Live proof `release_health_greptime.rs` (managed engine, OTLP v1 healthy + v2 crashing). Gap-matrix restamped to match (81/14 post-R3-merge). |
