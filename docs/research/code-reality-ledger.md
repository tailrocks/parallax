...
| Claim | Status | In-repo pointer | Notes |
| --- | --- | --- | --- |
| GraphQL query API | **shipped** | `crates/parallax-api/`; exported schema `ui/graphql/schema.graphql` | **88** Query fields, **15** Mutation fields (R1+R3+R5+R2 merge restamp: R3 `rumSessions` + `rumSession`, R1 `sourceMaps` query + `sourceMapUpload` mutation + `ErrorEvent.mappedFrames`, R2 `samplingPolicy` + `ingestDrops` + `ingestQueues`, R5 `releaseHealth`; prior: audit-M1 restamp `chartAnnotations` new in PR71 `ec0eea1c`, 77→78; `service` is now optional). **Count method (SoT):** walk `type Query` / `type Mutation` in the generated SDL; **skip `"""…"""` description blocks** (and single-line `"` descriptions); then match `fieldName(` or `fieldName:`. A naive line regex that does **not** skip descriptions falsely reports extra fields. Re-count before changing this row. `chartAnnotations(fromNanos, toNanos, service: Option)` — omit service collects ≤32; kind `release`, title=version. Trace field `dominantDbQueries`. `releaseHealth(service, fromNanos, toNanos)` — per-release crash-free session/user rates + `suspectRelease` flag. |
...
| Release health | **shipped** | GraphQL `releaseHealth`; storage `release_health` (memory + GreptimeDB); UI `ServiceReleaseStrip` crash-free + suspect badge | Per-service+version crash-free session/user rates from ingested spans + derived error events; `suspectRelease` when the session crash rate strictly regresses vs the previous release (first-seen order). Shared suspect rule `projections::flag_suspect_releases`. Live test `release_health_greptime.rs` (healthy v1 + crashing v2 via OTLP). |
...
| Sampling policy / drop reasons | **shipped** (R2 visibility slice) | GraphQL `samplingPolicy` / `ingestDrops` / `ingestQueues`; `[sampling]` config; UI `/pipeline` | Declared policy per signal (rule+rate, server leg keep-all) + live drop-reason counters + queue watermarks. No enforcement, no per-service overrides, no policy controls (future). |
...
| 2026-09-13 | R2 sampling visibility slice (`goal/r2-sampling`): declared `[sampling]` policy per signal + live `samplingPolicy` / `ingestDrops` / `ingestQueues` GraphQL readout + `/pipeline` UI page; `IngestHealth` accepted counters + per-signal `by_signal` loss JSON (additive). GraphQL **81→84** on the R1+R3 merge (SoT recount; R2 alone was 78→81), Mutation 15 holds. Live proof `r2_sampling_drops.rs` (in-process server, real OTLP+GraphQL). No enforcement, no per-service overrides, no policy controls. |
...
| 2026-09-14 | R5 release health: GraphQL row **78→82** (with R1+R3) — `releaseHealth` is new (per-service+version crash-free session/user rates + `suspectRelease` on crash-rate regression vs previous release). Mutation 15 with R1 (`sourceMapUpload`). Rollup reuses `serviceVersion`/error-event plumbing (`ReleaseHealth::from_counts`, `flag_suspect_releases` shared by memory + GreptimeDB); strip shows crash-free % + suspect badge. Live proof `release_health_greptime.rs` (managed engine, OTLP v1 healthy + v2 crashing). Gap-matrix restamped to match (82/15 post-R1-merge). |
