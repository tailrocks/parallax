...
| Upgrade breakage industry-wide; competitors' top-reacted issues are migration failures | Turso schema unversioned (plan 169 Step 4 adds `user_version`); **no cross-release data-dir upgrade test exists**; spool has legacy-reader compat (plan 114) | **plans/174** durable-upgrade + no-silent-loss contract |
| Silent event dropping at quota (Sentry); data loss/corruption (OpenObserve/Maple) | Spool = raw-frame forensic trail; `/health` 503 on ingest degradation; but the **no-silent-drop property is not stated or proven** as a contract | **plans/174** |
| Resource hunger / idle footprint (SigNoz ZK 800MB idle; 9GB collectors) | Single binary + managed GreptimeDB child; **no measured/published idle footprint, no regression gate** | **plans/175** footprint contract |
