...
> 19th UNFIRED**; **pass 202: 20th UNFIRED** — still **v0.2.0-alpha12 / 109★**;
> latest commits still GreptimeDB min **v1.1.3** probe/bump + session UI only —
> no Sentry/redaction/outcome/prod-error collision). Sources:
...
> a direct collision.** As of 2026-07-18 **pass 202**, **that trigger has not fired**
> (pass 98 code search still stands; pass 134/181/202 commit log re-scan: GreptimeDB
> min **v1.1.3** install probe + session UI only).
...
- ~~Exact latest TMA1 release~~ → **pinned v0.2.0-alpha12; 109★.** Watch triggers **15th UNFIRED** (…/62/63/**64**): still install/perf — **zero** prod-error / Sentry / redaction / outcome / deploy / fingerprint hits.
