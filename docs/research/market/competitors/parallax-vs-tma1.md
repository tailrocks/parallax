...
> An unbiased, one-to-one comparison. Research date: **2026-07-17** (pass 59: **12th consecutive watch UNFIRED** — still **v0.2.0-alpha12 / 109★**; recent = GreptimeDB install probe + session-detail perf).
...
- ~~Exact latest TMA1 release~~ → **pinned v0.2.0-alpha12; 109★.** Watch triggers **12th UNFIRED** (…/56/57/58/**59**): still install/perf (GreptimeDB probe + session-detail) — **zero** prod-error / Sentry / redaction / outcome / deploy / fingerprint hits in recent commits.
