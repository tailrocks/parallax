...
| **Rustrak** | **Rust** Sentry-SDK-compatible error tracker (Actix + Postgres); **@rustrak/mcp** "full control" (mutating issue tools); **GPL-3.0**; ~64★, server **0.9.2**. Error-only (no OTLP). | GPL-3.0 self-host. | Errors only (Rust Sentry-alt + MCP). | [deep-dive](parallax-vs-rustrak.md) |
