...
| **Bugsink** | Focused **self-hosted Sentry-SDK-compatible error-tracking server** (Python/Django; full issue lifecycle — grouping/resolve/regress/release); 1,939★, v2.4.0, active. Error-only (no OTLP). The cleanest "run your own Sentry." | Open-core (core + `ee/` + BSD-3 `sentry/` fork; `NOASSERTION`) + Cloud. | Errors only (Sentry-alternative). | [deep-dive](parallax-vs-bugsink.md) |
