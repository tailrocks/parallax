...
- **OTLP (pass 47 re-verify):** Sentry **does** ingest OTLP — **still open beta** (docs: “features in beta are still in-progress and may have bugs”). Supports **traces and logs** via OTLP; official docs state flatly **“Sentry does not support OTLP metrics at this time”** ([docs.sentry.io/concepts/otlp](https://docs.sentry.io/concepts/otlp/), 2026-07-17). Not OTLP-native storage (maps into envelope/span model + Snuba/ClickHouse). Self-hosted OTLP path exists since ~v25.8.0. **Metrics via Sentry SDK remain separate from OTLP.**
