...
- **OTLP (pass 47 + 56 + 59 + 63 + 65 + **96** + **143** + **183** + **205**
  re-verify):** Sentry **does** ingest OTLP — **still open beta**. Supports
  **traces and logs** via OTLP; official docs still state verbatim **“Sentry does
  not support OTLP metrics at this time”**
  ([docs.sentry.io/concepts/otlp](https://docs.sentry.io/concepts/otlp/), pass
  **205**). Self-host latest still **`26.7.0`** (GitHub `getsentry/self-hosted`,
  2026-07-16). **OTLP-metrics GA watch still UNFIRED.**
