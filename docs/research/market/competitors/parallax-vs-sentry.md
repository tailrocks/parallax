...
- **OTLP (pass 47 + 56 + 59 re-verify):** Sentry **does** ingest OTLP — **still open beta**. Supports **traces and logs** via OTLP; official docs still state **“Sentry does not support OTLP metrics at this time”** ([docs.sentry.io/concepts/otlp](https://docs.sentry.io/concepts/otlp/), 2026-07-17 pass 59). Direct OTLP nav = traces + logs only (no metrics child). Not OTLP-native storage. Self-hosted OTLP since ~v25.8.0. **OTLP-metrics GA watch UNFIRED pass 59.**
