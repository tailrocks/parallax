...
- **OTLP:** Sentry **does** ingest OTLP — but **traces and logs only; no OTLP metrics** ([official docs](https://docs.sentry.io/concepts/otlp/), confirmed 2026-07-17). Sentry is **not OTLP-native in storage**: OTLP data is mapped into Sentry's own envelope/span model and ClickHouse/Snuba backend. This is a genuine, current gap — if your stack standardizes on OTLP metrics, Sentry cannot ingest them.
- **Sentry envelope / DSN:** Sentry's native protocol. Parallax plans envelope *compatibility* (absorb Sentry's 30+ SDKs) — i.e. Parallax interoperates *with* Sentry's SDKs rather than competing on SDK authoring. This is a Parallax thesis, **planned, not shipped.**
- **SDK fleet:** Sentry ships 30+ language/platform SDKs (web, iOS, Android, React Native, Flutter, Unity, game consoles) — the broadest in error tracking. Parallax relies on OTel SDKs (plus the planned Sentry-envelope path). On SDK breadth, **Sentry wins decisively.**
