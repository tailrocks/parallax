...
| [Coroot AI RCA](https://docs.coroot.com/ai/overview/), [Coroot MCP](https://docs.coroot.com/mcp/overview/), and [Coroot eBPF tracing](https://docs.coroot.com/tracing/ebpf-based-tracing/) | Coroot AI RCA is Enterprise or Cloud-connected for Community users; Coroot exposes an MCP endpoint with topology, alerts, incidents, traces, logs, metrics, Community `resolve_alerts`, and Enterprise `investigate_anomaly`; eBPF traces may not provide complete traces. | Coroot has crossed the MCP trigger, but still lacks Parallax's Sentry migration and coding-agent side-effect audit, and its MCP is not purely read-only. |
...
| Coroot | `trigger_hit` | Official MCP endpoint is documented, including Community tools, Community `resolve_alerts`, and Enterprise `investigate_anomaly`. | eBPF traces can be incomplete; no Sentry migration, portable bundle, or coding-agent action/outcome audit. |
