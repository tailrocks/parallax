...
The material change in this pass is that Coroot now has official MCP
documentation. That hits an earlier watch trigger in
[Open self-hosted competitor watch](open-self-hosted-competitor-watch.md), but
it does not close the Parallax wedge because Coroot still does not show a
Sentry-compatible migration path, portable redacted evidence bundles, or
coding-agent side-effect/outcome audit in the public docs checked here.
...
| [OpenObserve SRE Agent setup](https://openobserve.ai/docs/administration/deployment/sre-agent-setup-guide/) and [OTLP docs](https://openobserve.ai/docs/ingestion/logs/otlp/) | The SRE Agent requires an OpenObserve Enterprise license and AI provider key; it uses OpenObserve MCP tool URLs and validation settings; OpenObserve supports OTLP/HTTP and OTLP/gRPC for logs, metrics, and traces. | OpenObserve is still the closest Rust/object-storage threat, but the agent layer is not an open free-tier wedge in the checked docs. |
| [SigNoz agent-native observability](https://signoz.io/agent-native-observability/) and [SigNoz MCP server](https://signoz.io/docs/ai/signoz-mcp-server/) | SigNoz positions observability inside coding agents, exposes traces/logs/metrics/topology/deploy history through MCP, and supports a local MCP server for self-hosted users. | SigNoz directly attacks the "agent-native observability" story. |
...
| [Coroot AI RCA](https://docs.coroot.com/ai/overview/), [Coroot MCP](https://docs.coroot.com/mcp/overview/), and [Coroot eBPF tracing](https://docs.coroot.com/tracing/ebpf-based-tracing/) | Coroot AI RCA is Enterprise or Cloud-connected for Community users; Coroot now exposes an MCP endpoint with topology, alerts, incidents, traces, logs, metrics, and Enterprise `investigate_anomaly`; eBPF traces may not provide complete traces. | Coroot has crossed the MCP trigger, but still lacks Parallax's Sentry migration and coding-agent side-effect audit. |
...
| OpenObserve | `wedge_under_pressure` | Rust, object-storage-oriented, OTLP, AI SRE, MCP validation, but SRE Agent requires Enterprise license and AI key. | No Sentry-envelope migration in checked docs; no open bundle/action-audit contract. |
| SigNoz | `wedge_under_pressure` | Self-hosted MCP and agent-native observability now directly target Claude Code, Codex, Cursor, and similar workflows. | No Sentry envelope path or deterministic evidence bundle/outcome graph in checked docs. |
| Coroot | `trigger_hit` | Official MCP endpoint is now documented, including Community tools and Enterprise `investigate_anomaly`. | eBPF traces can be incomplete; no Sentry migration, portable bundle, or coding-agent action/outcome audit. |
