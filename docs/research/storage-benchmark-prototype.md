...
Runs 140-145 store the four-build benchmark as code, define and validate the
local/server tier split, tighten schema guidance, and source-read GreptimeDB
TWCS/TTL mechanisms, but they do not measure mixed native ingest, Q6 p95/p99,
stale-bundle rate, object-store economics, ClickHouse LTS, or end-to-end A5
integration. Local `bench/four-way` defaults to `N=100000` after Run 143 and Run
145 validates that default as laptop-safe; `N=5000000+` belongs on a server and
