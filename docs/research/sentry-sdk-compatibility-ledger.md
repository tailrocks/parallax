...
| [Bugsink Sentry SDK compatibility](https://www.bugsink.com/sentry-sdk-compatible/), [Bugsink simplicity recheck](bugsink-sentry-compatible-simplicity-recheck.md), [Rustrak recheck](rustrak-sentry-mcp-protocol-recheck.md), and [Urgentry](https://urgentry.com/) | Lightweight competitors publicly use DSN-change or drop-in Sentry replacement language. Bugsink's current docs explicitly frame migration as keeping the SDK and updating the DSN. Rustrak is Rust-first and envelope-focused, but its own drift report says non-event Sentry items such as sessions, transactions, client reports, attachments, and spans are not stored. | Parallax needs more precise evidence and wording; simple compatibility language is already crowded, and "Sentry-compatible" must distinguish error-event envelopes from full Sentry protocol coverage. |
