...
> engine) and **tees** in-process to derive `error_event` rows + issues into custom extension tables —
> it no longer hand-rolls custom signal tables. GreptimeDB-only (ClickHouse deferred). The protocol /
> conformance strategy below is unchanged. Canonical: [../decisions/native-otel-tables.md](../decisions/native-otel-tables.md).
