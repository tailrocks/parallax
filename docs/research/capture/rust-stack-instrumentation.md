...
| GraphQL (Juniper 0.17.1) | **Nothing** — tracking issue #423 open since 2019; no per-operation/per-field spans | — | Manual `#[instrument]` on the operation entry + expensive resolvers (per-scalar-field spans = noise). This satisfies the operator's "how each part of the GraphQL was generated and how long" via resolver-level spans |
