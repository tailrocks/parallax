...
| --- | --- | --- |
| [GitHub Deployments API](https://docs.github.com/en/rest/deployments/deployments?apiVersion=2022-11-28) | Deployments are requests to deploy a ref, include environment/task/payload, and dispatch `deployment` events. GitHub keeps deployment execution outside GitHub. | A GitHub deployment is a strong change marker only when Parallax records the deployed SHA/ref, environment, status, and external deployment logs. |
