...
- redaction report;
- canonical bundle JSON with `schema_ref`, post-redaction `canonical_hash`,
  `projection_manifest`, and access policy for Arm C/D;
...
| C Parallax bundle | Repo plus bounded canonical bundle with evidence refs, edge strengths, missing-data warnings, redaction report, source-field status, and projection manifest. | Product arm. Must be generated by a deterministic template, even if hand-assembled. Markdown is only a projection. |
...
- evidence-bundle schema ref, canonicalization method, projection surfaces, MCP
  output-schema requirement, and projection-audit hash;
...
docs/research/bundle-value-eval/
  ...
  tasks/<task_id>/arm-c-projection-manifest.json
  tasks/<task_id>/arm-c-mcp-structured-content.json
