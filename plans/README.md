...
| [162](162-fanout-lab-backend-pins.md) | Pin every fan-out-lab and playground infra image at current latest stable | BLOCKED — SigNoz v0.137.0 removed `deploy/docker/docker-compose.yaml` (Foundry-only); overlay include gone. Pins landed. Smoke PASS: OpenObserve v0.92.0, Maple v0.0.18, Sentry 26.7.2, Parallax host sink. Do not invent Foundry rewrite (recheck 2026-08-15T02:58Z: latest now v0.137.1; main + v0.137.0/v0.137.1 compose paths still 404; deploy/docker gone) |
| [163](163-playground-example-upgrades.md) | Upgrade every playground example to current-latest instrumentation and re-verify emission | BLOCKED — SigNoz Foundry residue only. 4-sink live 2026-08-14 (OO/Maple/Parallax/Sentry); Java gRPC→Rotel PASS at agent 2.30.0 (adopted); README matrix re-dated. (recheck 2026-08-14T13:20Z) |
| [164](164-playground-feature-coverage.md) | Extend the playground until every Parallax feature has a scripted scenario | BLOCKED — SigNoz-arm compare residue. c1–c11 registered on playground PR #13; live EXIT 0 against Parallax host 2026-08-14 (recheck 2026-08-14T12:41Z) |
| [165](165-user-lens-comparison.md) | Run the full playground sweep and record a user-lens comparison across all backends | BLOCKED — SigNoz Foundry-only residue (v0.137.0 compose 404; recheck 2026-08-14T15:18Z). Maple/OpenObserve/Sentry user-lens tables already live in playground `VERIFICATION.md` (2026-08-14 dual-emission + honest PRODUCT-LIMITED cells) |
...
| [167](167-agent-browser-ui-verification.md) | Verify every Parallax UI surface with agent-browser — functional + responsive | BLOCKED — SigNoz Foundry-only residue. 21-route + teaching walk recorded 2026-08-14 (`agent-browser.log`, coverage-matrix). Remaining W5 display FAILs: MCP `check` CLI≢GraphQL JSON; clock-skew banner same-service; exemplar workbench `hasTraceLink=false`; default `/ecosystem` 24h `serviceMap` INTERNAL; `/invocations` unbounded `observedInvocations` INTERNAL |
