...
| `plans/` | Sole home for unfinished implementation plans, the active-only index, execution contract, compact `GOAL.md` orchestration brief, and current reference analysis. Terminal plans and the goal brief are deleted at program closure; durable completion evidence belongs under `docs/research/validation/` or in Git history. |
...
| `crates/` | The V1 Rust workspace (edition 2024): product crates `parallax-cli`, `parallax-server`, pure `parallax-analysis`, `parallax-evidence`, zero-copy `parallax-ingest`, GreptimeDB adapter `parallax-greptime`, Turso adapter `parallax-metadata`, raw-frame durability owner `parallax-spool`, port owner `parallax-storage`, `parallax-api`, `parallax-model`, and `parallax-proto`; cycle-safe auxiliary fixtures in `parallax-test-support`; isolated proof crate `parallax-mcp-spike`; and repository control plane `parallax-xtask`. Contracts live in [`docs/research/architecture/v1-implementation-spec.md`](docs/research/architecture/v1-implementation-spec.md); unfinished implementation work lives in [`plans/`](plans/). |
| `crates/*/facade.toml` | Sorted syntax-derived public root manifests refreshed and drift-checked by `cargo xtask facade`. |
| `crates/*/src/**/tests.rs`, `crates/*/src/**/tests/` | External child unit-test modules. Production files retain only `#[cfg(test)] mod tests;`; larger test surfaces use focused scenario includes so private access and stable test IDs are preserved. |
