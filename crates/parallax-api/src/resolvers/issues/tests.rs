...
#[tokio::test]
async fn issues_bundle_json_is_byte_stable_across_repeated_reads() -> anyhow::Result<()> {
    let store = Arc::new(MemoryStore::new());
    let context = context_with_memory(Arc::clone(&store)).await;
    ...
    anyhow::ensure!(error_messages(&second).is_empty(), "{second}");
    anyhow::ensure!(
        first.pointer("/data/bundle/json") == second.pointer("/data/bundle/json"),
        "wall-clock generated_at must not make sequential bundle json diverge"
    );
    anyhow::ensure!(
        first.pointer("/data/bundle/canonicalHash") == second.pointer("/data/bundle/canonicalHash"),
    );
    Ok(())
}
