...
class = "product"
tier = 5
dependencies = ["parallax-evidence", "parallax-metadata", "parallax-model", "parallax-redaction", "parallax-semconv", "parallax-server", "parallax-spool", "parallax-storage"]
...
Run `cargo nextest run -p parallax-cli --all-features --locked` for the narrow crate gate and `cargo xtask facade check` for
root-surface drift.
