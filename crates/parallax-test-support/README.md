...
class = "test-support"
dependencies = ["parallax-metadata", "parallax-model", "parallax-proto", "parallax-semconv", "parallax-storage"]
facade_roots = ["lib.rs"]
+++
...
Run `cargo nextest run -p parallax-test-support --all-features --locked` for the narrow crate gate and `cargo xtask facade check` for
root-surface drift.
