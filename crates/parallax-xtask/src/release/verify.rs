...
pub(super) const SYFT_VERSION: &str = "1.45.1";
...
fn verify_sbom(archive: &Path, archive_name: &str, digest: &str) -> Result<()> {
    ...
    .with_context(|| format!("parse CycloneDX SBOM {}", path.display()))?;
        ...
        document["serialNumber"]
            .as_str()
            .is_some_and(|serial| serial.starts_with("urn:uuid:") && serial.len() > 9),
        "SBOM serial number is missing or invalid"
    );
    ensure!(
        document["version"]
            .as_u64()
            .is_some_and(|version| version > 0),
        "SBOM document version is missing or invalid"
    );
    ensure!(
        document["metadata"]["timestamp"]
            .as_str()
            .is_some_and(|timestamp| !timestamp.is_empty()),
        "SBOM generation timestamp is missing"
    );
    ensure!(
        document["metadata"]["component"]["type"] == "file",
        "SBOM root component must describe the release archive as a file"
    );
    ensure!(
    ...
    let tools = document["metadata"]["tools"]["components"]
        .as_array()
        .context("SBOM generator inventory is missing")?;
    ensure!(
        tools.iter().any(|tool| {
        ...
        "SBOM was not generated by pinned Syft {SYFT_VERSION}"
    );
    Ok(())
}
