...
test.describe("investigations product pilot", () => {
  ...
  test("lists seeded investigation and opens detail @pw-investigations-list-detail", async ({
    ...
    await expect(screen.notesField()).toHaveValue(
      "Initial case notes from fixture seed."
    )
  ...
  test("create edit delete pin note and persist @pw-investigations-crud-pilot", async ({
    ...
    await expect(
      page.getByRole("heading", { name: "Mutation pilot case" })
    ).toBeVisible()
    ...
    expect(
      afterSave.investigations.some((row) => row.name === "Mutation pilot case")
    ).toBe(true)
    const created = afterSave.investigations.find(
      (row) => row.name === "Mutation pilot case"
    )
    ...
    await expect(screen.notesField()).toHaveValue(
      "Persisted note from browser contract"
    )
    ...
    expect(
      afterDelete.investigations.every(
        (row) => row.name !== "Mutation pilot case"
      )
    ).toBe(true)
    expect(afterDelete.investigations.some((row) => row.id === PILOT_ID)).toBe(
      true
    )
  })
})
