...
const HISTOGRAM_AGGS: readonly MetricAggregation[] = ["p50", "p95", "p99", "avg"]
const SUMMARY_AGGS: readonly MetricAggregation[] = ["p50", "p95", "p99", "avg"]

/** Legal aggregations for a metric kind (empty for unknown). */
export function legalAggregations(kind: MetricKind): readonly MetricAggregation[] {
  switch (kind) {
    case "sum":
      return SUM_AGGS
    case "gauge":
...
export function isLegalAggregation(kind: MetricKind, agg: MetricAggregation): boolean {
...
export function inferMetricKind(typeOrName: string | null | undefined): MetricKind {
...
export function decodeMetricQuerySpec(
  ...
  const aggregation = coerceAggregation(kind === "unknown" ? "gauge" : kind, get("agg"))
  ...
  const validStep =
    stepSeconds != null && Number.isFinite(stepSeconds) && stepSeconds > 0 ? stepSeconds : undefined
  return {
    name,
