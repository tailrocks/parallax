...
describe("ZOOM", () => {
  it("keeps the ms under the cursor fixed (anchor invariance)", () => {
    ...
    const before =
      (anchorMs - state.viewport.startMs) / (state.viewport.endMs - state.viewport.startMs)
    const after = (anchorMs - next.viewport.startMs) / (next.viewport.endMs - next.viewport.startMs)
  ...
  it("stops zooming in at the minimum window", () => {
    ...
    expect(next.viewport.endMs - next.viewport.startMs).toBeCloseTo(MIN_VISIBLE_MS, 10)
  })
  ...
  it("ignores non-positive or non-finite factors", () => {
    ...
    expect(timelineReducer(state, { type: "ZOOM", factor: 0, anchorMs: 50 })).toBe(state)
    expect(
...
describe("ZOOM_TO_FIT and secondary actions", () => {
  ...
  it("sets search text", () => {
    ...
    expect(timelineReducer(state, { type: "SET_SEARCH", search: "db" }).search).toBe("db")
  })
