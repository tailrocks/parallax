...
import {
  ...
  OutcomeChip,
} from "@/features/invocations/components/invocation-status-badge"
...
export function InvocationHubPage({
  invocationId,
  data,
  search,
}: {
  invocationId: string
  data: Awaited<ReturnType<typeof loadInvocationHub>>
  search: HubSearch
}) {
  const navigate = useNavigate({ from: "/invocations/$invocationId" })
