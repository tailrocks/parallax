...
import type { Invocation, ObservedInvocation } from "@/features/invocations/model/wire"
...
export function invocationStatus(
  ...
  const failedOutcome =
    row.outcome === "failure" || row.outcome === "error" || row.outcome === "timeout"
  ...
  const lastSeen = BigInt(
    row.lastNanos !== "0" && row.lastNanos !== "" ? row.lastNanos : row.startedAtNanos
  )
...
export function mergeInvocations(
  ...
  for (const row of invocations) {
    ...
    rows.set(row.invocationId, {
      ...
      lastNanos: maxNanos(seen?.lastNanos, row.endedAtNanos ?? row.startedAtNanos),
  ...
  return [...rows.values()].sort((a, b) => (BigInt(a.lastNanos) < BigInt(b.lastNanos) ? 1 : -1))
}
