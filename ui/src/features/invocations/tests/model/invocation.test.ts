...
describe("invocationStatus", () => {
  it("derives the full matrix", () => {
    ...
    expect(invocationStatus({ ...base, lastNanos: nanos(-301) }, NOW_MS)).toBe("stale")
    expect(invocationStatus({ ...base, endedAtNanos: nanos(-10), exitCode: 0 }, NOW_MS)).toBe(
      "finished"
    )
    expect(invocationStatus({ ...base, endedAtNanos: nanos(-10), exitCode: 3 }, NOW_MS)).toBe(
      "failed"
    )
    expect(
      invocationStatus(
    ...
    expect(invocationStatus({ ...base, outcome: "failure" }, NOW_MS)).toBe("failed")
  })
})
...
describe("invocationDurationNs", () => {
  it("uses now while running and end when finished", () => {
    ...
    expect(invocationDurationNs(row, "finished", NOW_MS)).toBe((60n * 1_000_000_000n).toString())
    expect(invocationDurationNs({ ...row, endedAtNanos: null }, "running", NOW_MS)).toBe(
      (100n * 1_000_000_000n).toString()
    )
  })
})
