...
describe("colorForSpan", () => {
  it("service mode uses the plan-162 deterministic service color", () => {
    expect(colorForSpan({ kind: "service" }, span())).toBe(serviceColor("checkout").color)
    expect(colorForSpan({ kind: "service" }, span({ service: "" }))).toBe(COLOR_BY_UNKNOWN)
  })
  ...
  it("span-kind mode maps the five OTLP kinds and defaults unknown", () => {
    ...
    const colors = kinds.map((kind) => colorForSpan({ kind: "spanKind" }, span({ kind })))
    expect(new Set(colors).size).toBe(kinds.length)
    expect(colorForSpan({ kind: "spanKind" }, span({ kind: "SPAN_KIND_UNSPECIFIED" }))).toBe(
      COLOR_BY_UNKNOWN
    )
  })
...
describe("attributeKeysForColorBy", () => {
  it("returns sorted unique keys across all spans", () => {
    ...
    expect(attributeKeysForColorBy(spans)).toEqual(["db.system", "http.method", "rpc.service"])
  })
})
...
describe("colorByLegend", () => {
  it("lists distinct labels in first-seen order", () => {
    ...
    expect(colorByLegend({ kind: "service" }, spans).map((e) => e.label)).toEqual([
      "checkout",
      "cart",
    ])
  })
  ...
  it("labels missing attribute values and caps the list", () => {
    ...
    const legend = colorByLegend({ kind: "attribute", key: "http.route" }, spans)
