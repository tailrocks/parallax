...
describe("parseWhereClause", () => {
  ...
  it("AND chain with mixed operators", () => {
    expect(
      parseWhereClause('service = "checkout" AND attr.http.route != "/health" AND duration > 100')
    ).toEqual({
      ok: true,
      filters: [
  ...
  it("case-insensitive keywords", () => {
    expect(parseWhereClause('a = 1 and b contains "x" AND c not contains "y"')).toEqual({
      ok: true,
      filters: [
        { key: "a", op: "=", value: "1" },
...
describe("whereClauseFromSearch", () => {
  ...
  it("valid clause parses", () => {
    expect(whereClauseFromSearch("a = 1")).toEqual([{ key: "a", op: "=", value: "1" }])
  })
})
