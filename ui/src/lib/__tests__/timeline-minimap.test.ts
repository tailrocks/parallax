...
describe("minimapDragAction", () => {
  it("pans by full-trace minimap scale", () => {
    expect(
      minimapDragAction(
        { hit: "pan", originPx: 100, viewport },
        150,
        500,
        1_000
      )
    ).toEqual({ type: "PAN", deltaMs: 100 })
  })

  it("resizes either viewport edge", () => {
    expect(
      minimapDragAction(
        { hit: "resize-start", originPx: 100, viewport },
        50,
        500,
        1_000
      )
    ...
    expect(
      minimapDragAction(
        { hit: "resize-end", originPx: 200, viewport },
        350,
        500,
        1_000
      )
