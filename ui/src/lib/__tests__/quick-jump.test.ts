...
describe("guessId", () => {
  ...
  it("recognizes real-shaped non-hex run ids from fixtures and playground", () => {
    expect(guessId(" run-a ")).toEqual([{ kind: "run", id: "run-a" }])
    expect(guessId("run_cli")).toEqual([{ kind: "run", id: "run_cli" }])
    expect(guessId("plan065-live-1783552201")).toEqual([
      { kind: "run", id: "plan065-live-1783552201" },
    ])
  })
