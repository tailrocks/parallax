...
export function alertRuleSaveMutation(
  ...
  const fields = [
    ...
    draft.thresholdUpper != null
      ? numberField("thresholdUpper", draft.thresholdUpper)
      : null,
    ...
    numberField(
      ...
      draft.consecutiveBreachesRequired
    ),
    ...
    draft.metricAggregation
      ? `metricAggregation: "${gqlString(draft.metricAggregation)}"`
      : null,
  ]
    .filter(Boolean)
    .join(", ")
