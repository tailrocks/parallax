...
| `objective-research-reality.md` | **Critical objectivity program:** re-read project state; keep `docs/research/` aligned with shipped `crates/`/`ui/` (code-reality ledger); **equal-skepticism grilling** of Parallax *and* competitors with the same questions; multi-angle economics and openness (price, TCO, contribute, lock-in, ecosystem); clean stale planned-vs-shipped language; prefer transparency over bias toward any side. Complements `parallax-vs-competitors.md` (market deep-dives) with code-authority discipline and symmetric grilling. |
...
Core method: equal-skepticism grilling — apply the same hard questions to
Parallax and every peer (shipped vs claimed, evidence quality, failure modes,
economics, openness, lock-in, falsifiers) before writing scoped verdicts.
...
highest-value gap (ledger drift, asymmetric skepticism, freestyle
...
navigation); grill the full comparison set symmetrically when the gap is
market-facing; fix or banner; update the ledger and a dated note under
